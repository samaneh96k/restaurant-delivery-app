...
import { useGetChefQuery } from '@/storeRedux/services/chefApi';

import React from 'react'


const AboutUsChef = () => {
    const { isLoading, isFetching, data, error } = useGetChefQuery(null);
      ...
      <>
          ...
          <div className=' flex flex-wrap '>
              {data?.map(chef => (
                  ...
                  <div key={chef.chefName} className='flex flex-wrap justify-center items-center  w-1/3  p-12 gap-4'>
                  <div className='aspect-w-2 aspect-h-2 w-full object-cover object-center '>
                      <img src={chef.chefImage} className='rounded'/>
                  </div>
                      <h2 className='text-2xl w-full text-center' >{ chef.chefName
}</h2>
                      <p className='color-icon w-full text-center'>{chef.chefTitle }</p>
              </div>
   ))}
